...
app.controller("MainController", ["$scope", "$http", '$window', function($scope, $http, $window) {
  ...
  $scope.proceed = function() {
    ...
    $http({
        url: 'http://ec2-52-201-184-155.compute-1.amazonaws.com:3000/api/note',
        method: "POST",
        data: mydata
    })
    .then(function(response) {
        $window.location.href = 'http://ec2-52-201-184-155.compute-1.amazonaws.com:3000/courses/' + $scope.coursecode + "/" + response.data;
    },
    function(response) {
        alert("great failure");
    });
  ...
  $scope.gettingClient = function() {
    ...
    $http({
      method: "GET",
          url: 'http://ec2-52-201-184-155.compute-1.amazonaws.com:3000/api/notes/course/' + $scope.coursecode.toUpperCase()
      })
      .then(function(response) {
  ...
  $scope.selectedObject = function(x) {
    if(x.originalObject.type === "coursecode") {
      $window.location.href = 'http://ec2-52-201-184-155.compute-1.amazonaws.com:3000/courses/' + x.originalObject.title;
    } else if(x.originalObject.type === "title") {
      $window.location.href = 'http://ec2-52-201-184-155.compute-1.amazonaws.com:3000/courses/' + x.originalObject.coursecode.toUpperCase() + "/" + x.originalObject.id;
    }
  }
